...
def profile_name(update: Update, context: CallbackContext) -> int:
...
def profile_bio(update: Update, context: CallbackContext) -> int:
...
def profile_age(update: Update, context: CallbackContext) -> int:
    """Сохраняем возраст и запрашиваем пол."""
    try:
        age = int(update.message.text)
        if age < 18 or age > 100:
            update.message.reply_text("⚠️ Пожалуйста, введите корректный возраст (18-100):")
            return AGE
        ...
        update.message.reply_text("⚠️ Пожалуйста, введите число:")
        return AGE

def profile_gender(update: Update, context: CallbackContext) -> int:
...
def search_gender_handler(update: Update, context: CallbackContext) -> int:
...
def profile_photo(update: Update, context: CallbackContext) -> int:
...
def show_matches(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text("💌 Ваши пары:")
...
def handle_like_dislike(update: Update, context: CallbackContext) -> None:
    ...
    try:
        if action == "like":
            ...
            query.answer("❤️ Вы поставили лайк!")
            ...
            query.answer("👎 Вы поставили дизлайк")
        ...
        query.answer("Произошла ошибка. Попробуйте еще раз.")
...
def update_search_preferences(update: Update, context: CallbackContext) -> int:
...
def handle_chat_button(update: Update, context: CallbackContext) -> int:
...
def handle_webapp_data(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if data.get('action') == 'like':
            ...
            update.message.reply_text("❤️ Вы поставили лайк!")
        elif data.get('action') == 'dislike':
            target_id = data.get('target_id')
            save_match(user_id, target_id, 'dislike')
            update.message.reply_text("👎 Вы пропустили профиль")
    except Exception as e:
...
def main() -> None:
    ...
    dp.add_handler(CallbackQueryHandler(handle_like_dislike, pattern="^(like|dislike)_"))
    dp.add_handler(CallbackQueryHandler(handle_chat_button, pattern="^chat_"))
